...
We can make a simple visualization of this:
...
The squads are stored in a list. Let's look at the first four rows of the first squad, which is Egypt:  
...
So, in the South Korean squad, there are two players who share birthdays on 27th February.
...
squads %>% map(dob_dup) %>% map(~filter(., dupl==T)) %>%
  map(~ select(., c(1:3, 5:9))) %>% data.table::rbindlist()
